...
//creating a class for storing the productions
class production
{
	public:
	char left;
...
//this function is used to separate the left and right part of the input
...
//separating the terminals apart (simply just checking small letters
...
//separating the non terminals apart (simply just checking big letters
...
//augmenting the grammar in order to parse 
			...
			if(refer[i].left==z)
			{
				...
				for(int j=0;j<refer[i].right.length();j++)
				{
					if(refer[i].right[j]=='.')
					{
						...
						//checking and putting the production if dot ke baad non terminal hai 
